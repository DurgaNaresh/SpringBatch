...
public class InsertData {
	...
	private static final String DB_CONNECTION = "jdbc:mysql://localhost:3306/toga";
	...
	private static void insertDataIntoUserTable() throws Exception {
		...
		try {
			...
			for (int i = 1; i <= 100; i++) {
				...
				String insertTableSQL = "INSERT INTO USER (id, username, password, age) "
						+ "VALUES (':id',':username',':password',':age')";

				insertTableSQL = insertTableSQL.replaceAll(":id", String.valueOf(i));
				insertTableSQL = insertTableSQL.replaceAll(":username", "user_" + i);
				insertTableSQL = insertTableSQL.replaceAll(":password", "pass_" + i);

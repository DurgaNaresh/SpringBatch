...
@ContextConfiguration(locations = {
    "classpath*:spring/batch/job/complex/aggregating/aggregating-items-in-writer-job.xml",
    "classpath*:spring/batch/setup/**/*.xml"})
@RunWith(SpringJUnit4ClassRunner.class)
public class AggregatingItemsInWriterJobConfigurationTest {
    ...
    public void launchJob() throws Exception {
        ...
        for (StepExecution step : jobExecution.getStepExecutions()) {
            assertEquals(
                    "Read count mismatch.",
                    testDataFactoryBean.getItemCount(),
                    step.getReadCount());
            assertEquals(
                    ...
                    step.getWriteCount());
            assertEquals(
                    "Real write count mismatch",
                    testDataFactoryBean.getAggregatedCount(),
                    step.getExecutionContext().getInt("real.write.count"));
        }
    }
}

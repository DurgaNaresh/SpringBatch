...
import com.github.daggerok.payment.PaymentsResource.Companion.canFail
...
class PaymentsResource(private val paymentsRepository: PaymentsRepository) {
    ...
    private companion object : KLogging() {
        val firstToBeFailed = AtomicBoolean(true)

        fun <T> T.canFail(method: String = ""): T = also {
            val currentTimeMillis = System.currentTimeMillis()
            if (firstToBeFailed.getAndSet(false) || currentTimeMillis % 2 == 0L) {
                logger.warn { "firstToBeFailed.getAndSet(false) || $currentTimeMillis % 2 == 0L" }
                throw RuntimeException("Testing retry for $method")
            }
        }
    }
}
